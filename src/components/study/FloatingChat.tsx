...
export function FloatingChat({ task, userId, buildContext }: FloatingChatProps) {
  ...
  return (
    <>
      {/* Chat panel — positioned below the button via JS rect */}
      <AnimatePresence>
        {open && (
          <>
            {/* Backdrop */}
            ...
            <motion.div
              ...
              className="fixed z-50 w-[340px] h-[460px] bg-card border border-border/60 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-border/40 bg-muted/20 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground/50 truncate max-w-[120px]">{task.title}</span>
                </div>
      ...
      {/* Inline button — rendered in header by parent */}
      <button
        ref={buttonRef}
      ...
      >
        ...
        {!open && messages.length > 0 && (
          <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-accent rounded-full text-[8px] font-bold flex items-center justify-center text-white">
