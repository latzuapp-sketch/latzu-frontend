...
import {
  ...
  Sparkles, X, Loader2, Brain, Clock, ArrowLeft,
} from "lucide-react";
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej: Notas del semestre"
              className="w-full rounded-xl border border-border/50 bg-muted/30 px-3.5 py-2.5 text-sm outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20"
            />
          </div>
          <div>
...
function NotebookCard({ deck, onDelete }: { deck: Deck; onDelete: (id: string) => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        <button
          onClick={() => { if (!confirmDel) { setConfirmDel(true); return; } onDelete(deck.id); }}
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-heading font-bold">Cuadernos</h1>
            <p className="text-sm text-muted-foreground mt-0.5">
              Organiza tus notas por tema o proyecto
            </p>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1 max-w-xs mx-auto">
              Organiza tus notas por tema, materia o proyecto.
            </p>
          </div>
      ...
      <AnimatePresence>
        {showCreate && (
          <CreateNotebookModal
            ...
            onCreate={async (name, desc, color) => { await createDeck(name, desc, color); }}
          />
        )}
      </AnimatePresence>
    </div>
